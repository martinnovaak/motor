...
void generate_pawn_moves(board & b, movelist & ml, uint64_t checkmask, uint64_t move_h, uint64_t move_v, uint64_t move_a, uint64_t move_d, uint64_t enemy, uint64_t empty) {
    ...
    const uint64_t pawns = b.get_pawn_bitboard();
    ...
    while(bitboard_push_1) {
        ...
        ml.add(move(pawn_square - up, pawn_square, our_color, PAWN));
    }

    while(bitboard_push_2) {
        int pawn_square = pop_lsb(bitboard_push_2);
        ml.add(move(pawn_square - up_2, pawn_square, our_color, PAWN, DOUBLE_PAWN_PUSH));
    }
    ...
    if (pawns_penultimate) {
        ...
        while (bitboard_promote_push) {
            ...
            ml.add(move(pawn_square - up, pawn_square, our_color, PAWN, QUEEN_PROMOTION));
            ml.add(move(pawn_square - up, pawn_square, our_color, PAWN, ROOK_PROMOTION));
            ml.add(move(pawn_square - up, pawn_square, our_color, PAWN, BISHOP_PROMOTION));
            ml.add(move(pawn_square - up, pawn_square, our_color, PAWN, KNIGHT_PROMOTION));
        }
        ...
        while (bitboard_promote_antidiagonal) {
            ...
            ml.add(move(pawn_square - antidiagonal_capture, pawn_square, our_color, PAWN, QUEEN_PROMOTION_CAPTURE, attacked_piece));
            ml.add(move(pawn_square - antidiagonal_capture, pawn_square, our_color, PAWN, ROOK_PROMOTION_CAPTURE, attacked_piece));
            ml.add(move(pawn_square - antidiagonal_capture, pawn_square, our_color, PAWN, BISHOP_PROMOTION_CAPTURE, attacked_piece));
            ml.add(move(pawn_square - antidiagonal_capture, pawn_square, our_color, PAWN, KNIGHT_PROMOTION_CAPTURE, attacked_piece));
        }

        while (bitboard_promote_diagonal) {
            int pawn_square = pop_lsb(bitboard_promote_diagonal);
            PieceType attacked_piece = b.get_piece(pawn_square);
            ml.add(move(pawn_square - diagonal_capture, pawn_square, our_color, PAWN, QUEEN_PROMOTION_CAPTURE, attacked_piece));
            ml.add(move(pawn_square - diagonal_capture, pawn_square, our_color, PAWN, ROOK_PROMOTION_CAPTURE, attacked_piece));
            ml.add(move(pawn_square - diagonal_capture, pawn_square, our_color, PAWN, BISHOP_PROMOTION_CAPTURE, attacked_piece));
            ml.add(move(pawn_square - diagonal_capture, pawn_square, our_color, PAWN, KNIGHT_PROMOTION_CAPTURE, attacked_piece));
        }
    }
    ...
    while(bitboard_left_capture) {
        ...
        ml.add(move(pawn_square - antidiagonal_capture, pawn_square, our_color, PAWN, CAPTURE, b.get_piece(pawn_square)));
    }

    while(bitboard_right_capture) {
        int pawn_square = pop_lsb(bitboard_right_capture);
        ml.add(move(pawn_square - diagonal_capture, pawn_square, our_color, PAWN, CAPTURE, b.get_piece(pawn_square)));
    }
    ...
    if (enpassant_square != N_SQUARES)
    {
        ...
        if(enpassant_antidiagonal && b.check_legality_of_enpassant<their_color>(enpassant_square - antidiagonal_capture, enpassant_square - up)) {
            ml.add(move(enpassant_square - antidiagonal_capture, enpassant_square, our_color, PAWN, EN_PASSANT, PAWN));
        }
        ...
        if(enpassant_diagonal && b.check_legality_of_enpassant<their_color>(enpassant_square - diagonal_capture, enpassant_square - up)) {
            ml.add(move(enpassant_square - diagonal_capture, enpassant_square, our_color, PAWN, EN_PASSANT, PAWN));
        }
        ...
        while (b1) {
            ...
            if (b.check_legality_of_enpassant<their_color>(square_from, enpassant_square - up)) {
                ml.add(move(square_from, enpassant_square, our_color, PAWN, EN_PASSANT, PAWN));
            }
        }
        */
    }
...
void generate_slider_moves(board & b, movelist & ml, uint64_t piece_bitboard, uint64_t checkmask, uint64_t enemy, uint64_t empty, uint64_t occupancy)
{
    while(piece_bitboard) {
        ...
        while(captures) {
            ...
            ml.add(move(square, target_square, our_color, pt, CAPTURE, b.get_piece(target_square)));
        }

        while(quiets) {
            int target_square = pop_lsb(quiets);
            ml.add(move(square, target_square, our_color, pt));
        }
    }
}
...
{
    uint64_t piece_bitboard = b.get_knight_bitboard() & pin; // pinned knight cannot move
    while(piece_bitboard) {
        int square = pop_lsb(piece_bitboard);
        ...
        while(captures) {
            int target_square = pop_lsb(captures);
            ml.add(move(square, target_square, our_color, KNIGHT, CAPTURE, b.get_piece(target_square)));
        }
        uint64_t quiets = attack_bitboard & empty;
        while(quiets) {
            int target_square = pop_lsb(quiets);
            ml.add(move(square, target_square, our_color, KNIGHT));
        }
    }
}
...
void generate_king_moves(board &b, movelist & ml, int king_square, uint64_t safe_squares, uint64_t empty, uint64_t enemy)
{
    ...
    while(king_quiets) {
        ...
        ml.add(move(king_square, target_square, our_color, KING));
    }

    uint64_t king_captures = king_moves & enemy;
    while (king_captures) {
        int target_square = pop_lsb(king_captures);
        ml.add(move(king_square, target_square, our_color, KING, CAPTURE, b.get_piece(target_square)));
    }
}
...
void generate_castle_moves(movelist & ml, int castling_right, uint64_t safe_squares, uint64_t empty) {
    ...
    {
        ml.add(move(king_e_square, king_g_square, our_color, KING, KING_CASTLE));
    }
    ...
    {
        ml.add(move(king_e_square, king_c_square, our_color, KING, QUEEN_CASTLE));
    }
}
...
void generate_all_moves(board & b, movelist & ml) {
    ...
    auto [friendly_pieces, enemy_pieces, occupancy, empty, checkers] = b.get_bitboards<our_color, enemy_color>();
    ...
    auto [pin_hv, pin_ad, move_h, move_v, move_a, move_d] = b.get_pinners<our_color, enemy_color>(king_square);
    auto [rook_bitboard, bishop_bitboard, queen_bitboard] = b.get_slider_bitboards();
    ...
    generate_pawn_moves<our_color>(b, ml, checkmask, move_h, move_v, move_a, move_d, enemy_pieces, empty);
