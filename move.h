...
public:
    ...
    constexpr move(int from, int to, Color color, int moving_piece, int type_information, int captured_piece) {
        m_move = from | to << 6 | color << 12 | moving_piece << 13 | type_information << 16 | captured_piece << 20;
    }

    constexpr move(int from, int to, Color color, int moving_piece, int type_information) {
        m_move = from | to << 6 | color << 12 | moving_piece << 13 | type_information << 16;
    }

    constexpr move(int from, int to, Color color, int moving_piece) {
