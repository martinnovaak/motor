...
constexpr unsigned int HIDDEN_SIZE = 1280;
...
struct Weights {
    std::array<std::array<std::array<std::array<std::array<std::int16_t, HIDDEN_SIZE>, 64>, 6>, 2>, 8> feature_weight;
...
public:
    ...
    void update_accumulator(const Piece piece, const Color color, const Square square, int wking, int bking) {
        const auto& white_weights = weights.feature_weight[buckets[wking] % 8][color][piece][get_square_index(square, wking)];
        const auto& black_weights = weights.feature_weight[buckets[bking ^ 56] % 8][color ^ 1][piece][get_square_index(square, bking) ^ 56];
