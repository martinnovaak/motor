...
std::int16_t alpha_beta(board& chessboard, search_data& data, std::int16_t alpha, std::int16_t beta, std::int8_t depth, bool cutnode) {
    ...
    if (data.singular_move == 0) {
        if (!(in_check || !chessboard.is_quiet(best_move)
            ...
            int weight = std::min(depth + depth * depth, 224);
            ...
            material_entry = std::clamp(material_entry, -16384, 16384);
            ...
            white_nonpawn_entry = std::clamp(white_nonpawn_entry, -16384, 16384);
            ...
            black_nonpawn_entry = std::clamp(black_nonpawn_entry, -16384, 16384);
        }
