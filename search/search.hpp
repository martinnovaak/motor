        ...
        if (!in_check && std::abs(beta) < 9'000) {
            ...
            if (node_type != NodeType::Null && depth >= nmp_depth && eval >= beta && static_eval >= beta && !chessboard.pawn_endgame()) {
                ...
                if (nullmove_score >= beta) {
                    return nullmove_score > 10'000 ? beta : nullmove_score;
                }
            }

            const auto probcut_beta = beta + 350;
            if (depth >= 7 && !(tt_hit && tt_entry.depth > depth - 3 && tt_entry.score < probcut_beta)) {
                ...
                for (std::uint8_t moves_searched = 0; moves_searched < movelist.size(); moves_searched++) {
                    ...
                    if (score >= probcut_beta) {
                        tt[zobrist_key] = {Bound::LOWER, std::int8_t(depth - 3), score, raw_eval, chessmove, zobrist_key};
                        return score;
                    }
                }
            }
