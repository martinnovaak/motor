...
std::int16_t correct_eval(const board & chessboard, int material_key, int threat_key, int raw_eval) {
    ...
    const int nonpawn_major_entry = nonpawn_correction_table_major[color][White][wkey_major % 16384] + nonpawn_correction_table_major[color][Black][bkey_major % 16384];
    return raw_eval + (entry * 2 + material_entry + threat_entry + nonpawn_minor_entry / 2 + nonpawn_major_entry / 2) / (256 * 3);
}
