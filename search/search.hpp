        ...
        if (score > best_score) {
            ...
            if (score > alpha) {
                ...
                if (alpha >= beta) {
                    ...
                    const int gedas_dream = (moves_searched > 3) && (static_eval + 80 < raw_eval) && (static_eval < old_alpha);
                    history->update<color, is_root>(data, chessboard, best_move, quiets, captures, depth, material_key % 512, gedas_dream);
                    break;
                }
            }
        }
