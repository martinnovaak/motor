    ...
    for (std::uint8_t moves_searched = 0; moves_searched < movelist.size(); moves_searched++) {
        ...
        if constexpr (!is_root) {
            if (moves_searched && best_score > -9'000 && movelist[moves_searched] < 15'000) {
                if (is_quiet) {
                    if (quiets.size() > lmp_base + (depth + in_check) * (depth + in_check) / (2 - improving)) {
                        continue;
                    }

                    int lmr_depth = std::max(0, depth - reduction - !improving + in_check + movelist.get_move_score(moves_searched) / 6000);
                ...
                int see_margin = is_quiet ? -see_quiet * (depth + in_check) : -see_noisy * (depth + in_check) * (depth + in_check);
