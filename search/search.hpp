...
std::int16_t alpha_beta(board& chessboard, search_data& data, std::int16_t alpha, std::int16_t beta, std::int8_t depth, bool cutnode) {
    ...
    if (data.singular_move == 0) {
        if (!(in_check || !chessboard.is_quiet(best_move)
            ...
            int diff = (best_score - raw_eval) * 1024;
            int weight = std::min(depth * depth + 2 * depth + 1, 128);

            int & entry = correction_table[color][chessboard.get_pawn_key() % 16384];
            entry = (entry * (1024 - weight) + diff * weight) / 256;
            entry = std::clamp(entry, -16'384, 16'384);

            int & material_entry = material_correction_table[color][material_key % 32768];
            material_entry = (material_entry * (1024 - weight) + diff * weight) / 256;
            ...
            white_nonpawn_entry = (white_nonpawn_entry * (1024 - weight) + diff * weight) / 256;
            white_nonpawn_entry = std::clamp(white_nonpawn_entry, -16'384, 16'384);

            int & black_nonpawn_entry = nonpawn_correction_table[color][Black][bkey % 16384];
            black_nonpawn_entry = (black_nonpawn_entry * (1024 - weight) + diff * weight) / 256;
            black_nonpawn_entry = std::clamp(black_nonpawn_entry, -16'384, 16'384);
        }
