...
public:
    ...
    void update(search_data &data, board &chessboard, const chess_move &best_move, move_list &quiets, move_list &captures, int depth, int material_key) {
        ...
        for (const auto &capture : captures) {
            ...
            bool threat_from = (threats & bb(capture.get_from()));
            bool threat_to = (threats & bb(cap_to));
            update_cap_history(capture_table[threat_from][threat_to][chessboard.get_piece(capture.get_from())][cap_to][chessboard.get_piece(cap_to)], penalty);
        }
    }
    ...
    int get_capture_score(board& chessboard, Piece from_piece, Square from, Square to, Piece to_piece) const {
        std::uint64_t threats = chessboard.get_threats();
        bool threat_from = (threats & bb(from));
        bool threat_to = (threats & bb(to));
