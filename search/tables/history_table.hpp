...
TuningOption pawn_weight("pawn_weight", 156, 0, 600);
TuningOption nonpawn_weight("nonpawn_weight", 124, 0, 600);
TuningOption threat_weight("threat_weight", 64, 0, 600);
TuningOption minor_weight("minor_weight", 154, 0, 600);
TuningOption concorrde_weight("concorrde_weight", 90, 0, 600);
TuningOption concorrde2_weight("concorrde2_weight", 120, 0, 600);
TuningOption concorrde3_weight("concorrde3_weight", 106, 0, 600);
...
public:
    ...
    void update_correction_history(board& chessboard, const search_data &data, int best_score, int raw_eval, int depth) {
        ...
        if (data.get_ply() > 1) {
            ...
            if (data.get_ply() > 2) {
                ...
                int &cont_entry2 = continuation_correction_table[prev3.piece_type][prev3.to][prev1.piece_type][prev1.to];
                ...
                if (data.get_ply() > 3) {
                    ...
                    int &cont_entry3 = continuation_correction_table[prev4.piece_type][prev4.to][prev1.piece_type][prev1.to];
    ...
    std::int16_t correct_eval(const board &chessboard, const search_data &data, int raw_eval) {
        ...
        if (data.get_ply() > 1) {
            ...
            if (data.get_ply() > 2) {
                auto prev3 = data.prev_moves[data.get_ply() - 2];
                cont_entry2 = continuation_correction_table[prev3.piece_type][prev3.to][prev1.piece_type][prev1.to];
                if (data.get_ply() > 3) {
                    auto prev4 = data.prev_moves[data.get_ply() - 3];
                    cont_entry3 = continuation_correction_table[prev4.piece_type][prev4.to][prev1.piece_type][prev1.to];
                }
            }
        }
