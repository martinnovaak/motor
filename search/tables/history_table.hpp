...
public:
    History()
            : history_table({}), continuation_table({}), capture_table({}),
              ...
              threat_correction_table({}), continuation_correction_table({}), continuation_correction_table2({}), corner_correction_table({}) {}
    ...
    void update_correction_history(board& chessboard, const search_data &data, int best_score, int raw_eval, int depth) {
        ...
        update_entry(corner_correction_table[color][chessboard.get_corner_key() % CORRECTION_TABLE_SIZE]);
    ...
    std::int16_t correct_eval(const board &chessboard, const search_data &data, int raw_eval) {
        ...
        const int corner_entry = corner_correction_table[color][chessboard.get_corner_key() % 16384];
        ...
        return raw_eval + (pawn_entry * 200 + threat_entry * 100 + nonpawn_entry * 200 + minor_entry * 150 + major_entry * 120 + cont_entry * 180 + cont_entry2 * 180 + corner_entry * 120) / (256 * 300);
    }
...
private:
    ...
    std::array<std::array<int, 16384>, 2> corner_correction_table;
