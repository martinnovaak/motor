...
public:
    History()
            : history_table({}), material_history_table({}), continuation_table({}), capture_table({}),
              ...
              major_correction_table({}), threat_correction_table({}), history_corretion_table({}) {}
    ...
    void update_correction_history(board& chessboard, const search_data &data, int best_score, int raw_eval, int depth) {
        ...
        if (data.get_ply()) {
            auto prev_move = data.prev_moves[data.get_ply() - 1];
            int &history_entry = history_corretion_table[color][prev_move.from][prev_move.to];
            history_entry = (history_entry * (256 - weight) + diff * weight) / 256;
            history_entry = std::clamp(history_entry, -8'192, 8'192);
        }
    }
    ...
    std::int16_t correct_eval(const board& chessboard, const search_data &data, int raw_eval) {
        ...
        if (data.get_ply() > 0) {
            auto prev_move = data.prev_moves[data.get_ply() - 1];
            history_entry = history_corretion_table[color][prev_move.from][prev_move.to];
        }

        auto [wkey, bkey] = chessboard.get_nonpawn_key();
        const int nonpawn_entry = nonpawn_correction_table[color][White][wkey % 16384] + nonpawn_correction_table[color][Black][bkey % 16384];

        return raw_eval + (entry * 195 + threat_entry * 102 + nonpawn_entry * 117 + major_entry * 92 + minor_entry * 137 + history_entry * 100) / (256 * 300);
    }
...
private:
    ...
    std::array<std::array<std::array<int, 64>, 64>, 2> history_corretion_table;
