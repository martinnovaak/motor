...
public:
    History()
            : history_table({}), pawn_history_table({}), material_history_table({}), continuation_table({}), capture_table({}),
    ...
    void update(search_data &data, board &chessboard, const chess_move &best_move, move_list &quiets, move_list &captures, int depth) {
        ...
        const std::uint64_t material_key = chessboard.get_material_key() % 512;
        ...
        if (chessboard.is_quiet(best_move)) {
            ...
            update_history(material_history_table[color][material_key][piece][to], bonus);
            ...
            for (const auto &quiet : quiets) {
                ...
                update_history(material_history_table[color][material_key][qpiece][qto], penalty);
    ...
    int get_quiet_score(board &chessboard, const search_data &data, Square from, Square to, Piece piece) const {
        ...
        move_score += material_history_table[color][material_key][piece][to];
...
private:
    ...
    std::array<std::array<std::array<std::array<int, 64>, 7>, 512>, 2> material_history_table;
