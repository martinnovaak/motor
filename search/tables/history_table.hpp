...
public:
    ...
    void update_correction_history(board& chessboard, const search_data &data, int best_score, int raw_eval, int depth) {
        ...
        if (data.get_ply() > 1) {
            ...
            int &neighbor_entry = neighbor_correction_table[prev1.piece_type][prev1.to][chessboard.get_neighbor_key(prev1.to) % 1024];
            neighbor_entry = (neighbor_entry * (256 - weight) + diff * weight) / 256;
            neighbor_entry = std::clamp(neighbor_entry, -8'192, 8'192);
    ...
    std::int16_t correct_eval(const board &chessboard, const search_data &data, int raw_eval) {
        ...
        int cont_entry = 0;
        int cont_entry2 = 0;
        int neighbor_entry = 0;
        if (data.get_ply() > 1) {
            auto prev1 = data.prev_moves[data.get_ply() - 1];
            auto prev2 = data.prev_moves[data.get_ply() - 2];
            cont_entry = continuation_correction_table[prev2.piece_type][prev2.to][prev1.piece_type][prev1.to];
            neighbor_entry = neighbor_correction_table[prev1.piece_type][prev1.to][chessboard.get_neighbor_key(prev1.to) % 1024];
            if (data.get_ply() > 2) {
                auto prev3 = data.prev_moves[data.get_ply() - 3];
        ...
        return raw_eval + (entry * 200 + threat_entry * 100 + nonpawn_entry * 200 + minor_entry * 150 + major_entry * 120 + cont_entry * 180 + cont_entry2 * 180 + neighbor_entry * 120) / (256 * 300);
    }
...
private:
    ...
    std::array<std::array<std::array<int, 1024>, 64>, 7> neighbor_correction_table;
