...
TuningOption pawn_weight("pawn_weight", 193, 0, 600);
TuningOption nonpawn_weight("nonpawn_weight", 127, 0, 600);
TuningOption threat_weight("threat_weight", 80, 0, 600);
TuningOption major_weight("major_weight", 87, 0, 600);
TuningOption minor_weight("minor_weight", 144, 0, 600);
TuningOption concorrde_weight("concorrde_weight", 136, 0, 600);
...
public:
    ...
    std::int16_t correct_eval(const board &chessboard, const search_data &data, int raw_eval) {
        ...
        return raw_eval + (entry * pawn_weight.value + threat_entry * threat_weight.value + nonpawn_entry * nonpawn_weight.value + major_entry * major_weight.value + minor_entry * minor_weight.value + cont_entry * concorrde_weight.value) / (256 * 300);
    }
