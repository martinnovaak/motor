...
public:
    ...
    void update_correction_history(board& chessboard, const search_data &data, int best_score, int raw_eval, int depth) {
        ...
        int &center_entry = correction_table[color][chessboard.get_center_key() % 16384];
    ...
    std::int16_t correct_eval(const board &chessboard, const search_data &data, int raw_eval) {
        ...
        const int center_entry = correction_table[color][chessboard.get_center_key() % 16384];
        ...
        return raw_eval + (entry * 177 + threat_entry * 103 + nonpawn_entry * 166 + major_entry * 78 + minor_entry * 180 + cont_entry * 142 + center_entry * 112) / (256 * 400);
    }
...
private:
    ...
    std::array<std::array<int, 16384>, 2> center_correction_table;
