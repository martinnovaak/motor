...
public:
    History()
            : history_table({}), material_history_table({}), continuation_table({}), capture_table({}),
              ...
              threat_correction_table({}), continuation_correction_table({}), adeh_correction_table({}) {}
    ...
    void update_correction_history(board& chessboard, const search_data &data, int best_score, int raw_eval, int depth) {
        ...
        int& adeh_entry = adeh_correction_table[color][chessboard.get_adeh_key() % 16384];
        adeh_entry = (adeh_entry * (256 - weight) + diff * weight) / 256;
        adeh_entry = std::clamp(adeh_entry, -8'192, 8'192);
    }
    ...
    std::int16_t correct_eval(const board &chessboard, const search_data &data, int raw_eval) {
        ...
        const int adeh_entry = adeh_correction_table[color][chessboard.get_adeh_key() % 16384];
        ...
        return raw_eval + (entry * 190 + threat_entry * 85 + nonpawn_entry * 130 + minor_entry * 143 + cont_entry * 140 + adeh_entry * 90) / (256 * 300);
    }
...
private:
    ...
    std::array<std::array<int, 16384>, 2> adeh_correction_table;
