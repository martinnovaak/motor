...
public:
    History()
            : history_table({}), material_history_table({}), continuation_table({}), capture_table({}),
              correction_table({}), material_correction_table({}), nonpawn_correction_table({}), minor_correction_table({}), major_correction_table({}),
    ...
    template<Color color>
    void update_correction_history(board& chessboard, const search_data &data, int best_score, int raw_eval, int depth, std::uint64_t material_key) {
        ...
        int &material_entry = material_correction_table[color][material_key % 16384];
    ...
    template <Color color>
    std::int16_t correct_eval(const board &chessboard, const search_data &data, int raw_eval, std::uint64_t material_key) {
        if (std::abs(raw_eval) > 8'000) return raw_eval;
        std::uint64_t threat_key = murmur_hash_3(chessboard.get_threats() & chessboard.get_side_occupancy<color>());

        const int entry = correction_table[color][chessboard.get_pawn_key() % 16384];
        const int material_entry = material_correction_table[color][material_key % 16384];
        ...
        return raw_eval + (entry * 200 + material_entry * 100 + threat_entry * 100 + nonpawn_entry * 200 + minor_entry * 150 + major_entry * 120 + cont_entry * 180 + cont_entry2 * 180) / (256 * 300);
    }
