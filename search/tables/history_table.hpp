...
public:
    History()
            : history_table({}), material_history_table({}), continuation_table({}), capture_table({}),
              ...
              major_correction_table({}), threat_correction_table({}), countermove_correction_table({}) {}
    ...
    void update_correction_history(board& chessboard, const search_data &data, int best_score, int raw_eval, int depth) {
        ...
        history_move prev = data.get_ply() > 0 ? data.prev_moves[data.get_ply() - 1] : history_move();
        int &cm_entry = countermove_correction_table[data.get_ply()][prev.piece_type][prev.to];
        cm_entry = (cm_entry * (256 - weight) + diff * weight) / 256;
        cm_entry = std::clamp(cm_entry, -8'192, 8'192);
    }
    ...
    std::int16_t correct_eval(const board& chessboard, const search_data &data, int raw_eval) {
        ...
        history_move prev = data.get_ply() > 0 ? data.prev_moves[data.get_ply() - 1] : history_move();
        const int cm_entry = countermove_correction_table[data.get_ply()][prev.piece_type][prev.to];

        return raw_eval + (entry * 195 + threat_entry * 102 + nonpawn_entry * 117 + major_entry * 92 + minor_entry * 137 + cm_entry * 100) / (256 * 300);
    }
...
private:
    ...
    std::array<std::array<std::array<int, 64>, 7>, 96> countermove_correction_table;
