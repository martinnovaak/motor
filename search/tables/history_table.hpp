...
public:
    History()
            : history_table({}), material_history_table({}), continuation_table({}), capture_table({}),
              correction_table({}), nonpawn_correction_table({}), minor_correction_table({}), center_correction_table({}),
    ...
    std::int16_t correct_eval(const board &chessboard, const search_data &data, int raw_eval) {
        ...
        return raw_eval + (entry * 192 + threat_entry * 88 + nonpawn_entry * 134 + minor_entry * 146 + center_entry * 100 + cont_entry * 150) / (256 * 300);
    }
