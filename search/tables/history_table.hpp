...
public:
    History()
            : history_table({}), material_history_table({}), center_history_table({}), continuation_table({}), capture_table({}),
    ...
    void update(search_data &data, board &chessboard, const chess_move &best_move, move_list &quiets, move_list &captures, int depth, int material_key) {
        ...
        if (chessboard.is_quiet(best_move)) {
            ...
            update_history(center_history_table[chessboard.get_center_key() % 512][color][piece][to], bonus);
            ...
            for (const auto &quiet : quiets) {
                ...
                update_history(center_history_table[chessboard.get_center_key() % 512][color][qpiece][qto], penalty);
    ...
    int get_quiet_score(board &chessboard, const search_data &data, Square from, Square to, Piece piece, int material_key) const {
        ...
        move_score += center_history_table[chessboard.get_center_key() % 512][color][piece][to];
...
private:
    ...
    std::array<std::array<std::array<std::array<int, 64>, 7>, 2>, 512> center_history_table;
