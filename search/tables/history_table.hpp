...
public:
    ...
    void update_correction_history(board& chessboard, const search_data &data, int best_score, int raw_eval, int depth) {
        ...
        int bucket = color == White ? buckets[chessboard.get_king_square<White>()] : buckets[chessboard.get_king_square<Black>() ^ 56];

        int &entry = correction_table[bucket][color][chessboard.get_pawn_key() % 16384];
        ...
        int &threat_entry = threat_correction_table[bucket][color][threat_key % 32768];
        ...
        int &white_nonpawn_entry = nonpawn_correction_table[bucket][color][White][wkey % 16384];
        ...
        int &black_nonpawn_entry = nonpawn_correction_table[bucket][color][Black][bkey % 16384];
        ...
        int &major_entry = major_correction_table[bucket][color][chessboard.get_major_key() % 16384];
        major_entry = (major_entry * (256 - weight) + diff * weight) / 256;
        major_entry = std::clamp(major_entry, -8'192, 8'192);

        int &minor_entry = minor_correction_table[bucket][color][chessboard.get_minor_key() % 16384];
        ...
        if (data.get_ply() > 1) {
            ...
            int &cont_entry = continuation_correction_table[bucket][prev2.piece_type][prev2.to][prev1.piece_type][prev1.to];
    ...
    std::int16_t correct_eval(const board &chessboard, const search_data &data, int raw_eval) {
        ...
        const int entry = correction_table[bucket][color][chessboard.get_pawn_key() % 16384];
        const int threat_entry = threat_correction_table[bucket][color][threat_key % 32768];
        const int major_entry = major_correction_table[bucket][color][chessboard.get_major_key() % 16384];
        const int minor_entry = minor_correction_table[bucket][color][chessboard.get_minor_key() % 16384];

        auto [wkey, bkey] = chessboard.get_nonpawn_key();
        const int nonpawn_entry = nonpawn_correction_table[bucket][color][White][wkey % 16384] + nonpawn_correction_table[bucket][color][Black][bkey % 16384];

        int cont_entry = 0;
        if (data.get_ply() > 1) {
            auto prev1 = data.prev_moves[data.get_ply() - 1];
            auto prev2 = data.prev_moves[data.get_ply() - 2];
            cont_entry = continuation_correction_table[bucket][prev2.piece_type][prev2.to][prev1.piece_type][prev1.to];
        }
...
private:
    ...
    std::array<std::array<std::array<int, 16384>, 2>, 6> correction_table;
    std::array<std::array<std::array<std::array<int, 16384>, 2>, 2>, 6> nonpawn_correction_table;
    std::array<std::array<std::array<int, 16384>, 2>, 6> minor_correction_table;
    std::array<std::array<std::array<int, 16384>, 2>, 6> major_correction_table;
    std::array<std::array<std::array<int, 32768>, 2>, 6> threat_correction_table;
    std::array<std::array<std::array<std::array<std::array<int, 64>, 7>, 64>, 7>, 6> continuation_correction_table;
