...
std::uint64_t perft(position& p, int depth) {
    ...
    for (const auto move : ml) {
        make_move<side, false>(p, move);
        nodes += perft<next_side>(p, depth - 1);
        undo_move<side, false>(p, move);
    }
    return nodes;
}
...
std::uint64_t perft(position& p, int depth) {
    ...
    std::uint64_t res = b.get_side() == White ? perft<White>(p, depth) : perft<Black>(p, depth);
...
template <Color side>
std::uint64_t perft_debug(position& p, int depth) {
    board& b = p.chessboard;
    constexpr Color next_side = (side == White) ? Black : White;
    move_list ml;
    generate_all_moves<side, false>(b, ml);
    ...
    for (const auto & m : ml) {
        ...
        move_string += " ";
        make_move<side, false>(p, m);
        std::uint64_t nodes = perft<next_side>(p, depth - 1);
        moves.push_back(move_string += std::to_string(nodes));
        total_nodes += nodes;
        undo_move<side, false>(p, m);
    }
...
void perft_debug(position& p, int depth) {
    board& b = p.chessboard;
    auto start = std::chrono::steady_clock::now();
    std::uint64_t result = b.get_side() == White ? perft_debug<White>(p, depth) : perft_debug<Black>(p, depth);
    auto end = std::chrono::steady_clock::now();
    std::chrono::duration<double, std::milli> elapsed_milliseconds = end - start;
    std::cout << "Time: " << elapsed_milliseconds.count() << ", nodes: " << result;
}

void perft_loop(position& pos, int from, int to) {
    while (from <= to) {
        std::cout << from << ": " << perft(pos, from) << std::endl;
        from++;
    }
}
...
void perft_test() {
    ...
    for (const auto& [fen, results] : perft_tests) {
        ...
        for (auto expected_nodes : results) {
            int nodes;
            if (b.get_side() == White) {
                nodes = perft<White>(p, depth);
            } else {
                nodes = perft<Black>(p, depth);
            }
