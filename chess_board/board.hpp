...
public:
    ...
    void make_null_move()
    {
        ...
        state->wing_key = old_info->wing_key;
    ...
    [[nodiscard]] std::uint64_t get_wing_key() const {
        return state->wing_key.get_key();
    }
    ...
    void make_state(Piece captured_piece, chess_move played_move) {
        ...
        state->wing_key = old_state->wing_key;
    ...
    void update_hash(Color color, Piece piece, Square square) {
        ...
        if (bb(square) & 0x81c3e7e7c38100ull) {
            state->wing_key.update_psqt_hash(color, piece, square);
        }
