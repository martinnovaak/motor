...
struct board_info {
    ...
    std::array<zobrist, 2> square_key = {};
...
public:
    ...
    void fen_to_board(const std::string& fen) {
        ...
        state->square_key[White] = state->square_key[Black] = zobrist();
    ...
    void make_null_move()
    {
        ...
        state->square_key = old_info->square_key;
    ...
    [[nodiscard]] std::pair<std::uint64_t, std::uint64_t> get_square_key() const {
        return { state->square_key[White].get_key(), state->square_key[Black].get_key()};
    }
    ...
    void make_state(Piece captured_piece, chess_move played_move) {
        ...
        state->square_key = old_state->square_key;
    ...
    void update_hash(Color color, Piece piece, Square square) {
        ...
        if (bb(square) & 0x28142814281400ull) {
            state->square_key[White].update_psqt_hash(color, piece, square);
        }

        if (bb(square) & 0x14281428142800ull) {
            state->square_key[Black].update_psqt_hash(color, piece, square);
        }
